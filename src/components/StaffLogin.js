...
const StaffLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      console.log('Login - Firestore data:', docSnap.exists() ? docSnap.data() : 'No data');
  ...
  const handleRegister = async (e) => {
    ...
    try {
      ...
      const user = userCredential.user;
      await setDoc(doc(db, 'staff', user.uid), { username, email, formFilled: false }, { merge: true });
      navigate('/staff-form');
    } catch (err) {
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (docSnap.exists() && docSnap.data().formFilled === true) {
        ...
        if (!docSnap.exists()) {
          await setDoc(docRef, { email: user.email, displayName: user.displayName || username, formFilled: false }, { merge: true });
        }
        navigate('/staff-form');
      }
    } catch (err) {
  ...
  return (
    <div className="login-page">
      <div className={`login-container ${isRegistering ? 'registration-container' : ''}`}>
        <div className="login-title">{isRegistering ? 'CREATE STAFF ACCOUNT' : 'STAFF LOGIN'}</div>
